...
namespace Box4RWickPivot
{
    public class Box4RWickPivot : NETStrategy
    {
        public Box4RWickPivot()
            : base()
        {
            base.ProjectName = "Box4RWickPivot";
            base.Version = "Version 1";
            base.Comments = "Baseado na agressao de um box de renko na formacao do wick no box passado";
        }
        ...
        public TimeSpan sessionStart = new TimeSpan(12, 15, 00);

        [InputParameter("Termino das operacoes", 7, 1, 999)]
        public TimeSpan sessionEnd = new TimeSpan(20, 00, 00);
        ...
        public override void OnQuote()
        {
			...
			foreach (var ord in All_ord)
			{
				...
				if (ord.Side == Operation.Buy && CurrentData.GetPrice(PriceType.Close, 0) > entrada + 4 * Point || 
				    ord.Side == Operation.Sell && CurrentData.GetPrice(PriceType.Close, 0) < entrada - 4 * Point)
					Orders.CancelAll();
			}
